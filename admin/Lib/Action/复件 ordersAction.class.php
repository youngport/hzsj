    ...
    public function status()
    {
        ...
        switch ($action) {
            ...
            case 'sended':
                ...
                $this -> weixintishi($order_openid[0]['buyer_id']," 亲爱的".$order_openid[0]['wei_nickname']."，您购买的宝贝已经发货,商品正搭着 ".$kuaidigongsi." （快递单号 ".$data['invoice_no']."）星夜向您奔来，洗洗干净，在家里等着我~");
            ...
            case 'tradefinished':
                ...
                if($arr_kdzt[0]['zhuangtai']==3 && $arr_kdzt[0]['status']==3){
                    ...
                    $sql = "select k.quantity,g.fp_price,g.goodtype,o.buyer_id,o.order_sn from sk_orders o left join sk_order_goods k on k.order_id=o.order_id left join sk_goods g on g.id=k.goods_id where o.order_id='$id'";
...
//        }
    ...
    public  function  extstatus(){
        ...
        switch($acton)
        {
            case "agree":
                ...
                $this -> weixintishi($quopenid[0]['openid'],"亲爱的 ".$quopenid[0]['wei_nickname']."，您于 ".date('Y-m-d H:i:s',time())." 在和众商城申请提现 ".$quopenid[0]['totpop']." 元，已通过审核");
            ...
            case "disagree":
                ...
                $this -> weixintishi($quopenid[0]['openid'],"亲爱的 ".$quopenid[0]['wei_nickname']."，很遗憾的告诉您，您 ".date('Y-m-d H:i:s',time())." 提现 ".$quopenid[0]['totpop']." 元，提现失败。您的现金已经被小怪兽吃掉了，赶快联系工作人员去看看马尼（money）去哪了~");
            ...
            $quopenid = $mod->table('wei_exchange')->join('sk_member on sk_member.open_id=wei_exchange.openid')->where(array('id'=>$id))->select();
            $this -> weixintishi($quopenid[0]['openid'],"亲爱的 ".$quopenid[0]['wei_nickname']."，恭喜您于 ".date('Y-m-d H:i:s',time())." 从和众世纪提现 ".$quopenid[0]['totpop']." 元，提现成功。聚少成多，金山银海也不过如此，每一分收益都是汗水浇灌的，所以。。。大神你该洗洗澡了orz");
            break;
        }
        if($result)
    ...
    public function order_shenfenz(){
    	$order = M('orders');
    	$map['order_id'] = array('eq',$_POST['id']);
    	$map_x['buyer_name'] = $_POST['xingmz'];
    	$map_x['shenfenz'] = $_POST['shenfzh'];
    	$data = $order -> where($map) -> save($map_x);
    	echo $data;
    }
    ...
    function weixintishi($openidd,$weixintishi_text){
        ...
        if((time() - $token_txt[0]['time'])>1800){//数据库里 微信token 已过期  需要跟新
            $ch = curl_init();

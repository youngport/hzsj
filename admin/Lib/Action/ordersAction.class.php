...
class ordersAction extends baseAction {
    ...
    public function index() {
        ...
        $status_txt = isset($_GET['status_txt']) && trim($_GET['status_txt']) ? trim($_GET['status_txt']) : '';
        ...
        if ($status_txt) {
            $where .= " AND  (status=1 or  status=2 or status=3 or status=4 or status=5)";
        }
        if ($buyer_name) {
        ...
        if ($fenlei == 0) {//普通订单
            $where .= " AND erm=" . $fenlei . " and is_member= 0 AND jifendh = 0";
        }
        if ($fenlei == 1) {//店铺订单
        ...
        if ($fenlei == 2) {//会员订单
            $where .= " AND (erm=" . $fenlei . " or is_member = 1)";
        }
        ...
        if (!empty($_GET['daochu'])) {
            $order_excel = $items_mod->field("*,(select sum(pop) from wei_pop where order_sn= sk_orders.order_sn) fp_price")->where($where)->join("sk_order_extm on sk_orders.order_id = sk_order_extm.order_id ")->join("sk_order_goods on sk_orders.order_id = sk_order_goods.order_id")->order($order_str)->select();
//             var_dump($order_excel); exit;
            ...
            $excel_canshu->getColumnDimension('Q')->setWidth(60);
            $excel_canshu->getColumnDimension('R')->setWidth(60);
            ...
            $this_biao->setCellValue('A1', '订单流水号')
                    ...
                    ->setCellValue('G1', '分出总额')
                    ->setCellValue('H1', '收货地址')
                    ->setCellValue('I1', '下单时间')
                    ->setCellValue('J1', '商品名称')
                    ->setCellValue('K1', '商品数量')
                    ->setCellValue('L1', '商品规格')
                    ->setCellValue('M1', '价格（单位：元）')
                    ->setCellValue('N1', '商品编号')
                    ->setCellValue('O1', '快递单号')
                    ->setCellValue('P1', '身份证号')
                    ->setCellValue('Q1', '买家附言')
                    ->setCellValue('R1', '购买人openid_id');
            $tong_id = "";
            $tong_id_y = 2;
            for ($i = 0; $i < count($order_excel); $i++) {
                $this_biao->setCellValue('J' . ($i + 2), $uphtml->uphtml_f($order_excel[$i]['goods_name']))
                        ->setCellValue('K' . ($i + 2), $order_excel[$i]['quantity'])
                        ->setCellValue('L' . ($i + 2), $order_excel[$i]['specification'] . " ")
                        ->setCellValue('M' . ($i + 2), $order_excel[$i]['price'] . " * " . $order_excel[$i]['quantity'] . " = " . ($order_excel[$i]['quantity'] * $order_excel[$i]['price']))
                        ->setCellValue('N' . ($i + 2), $order_excel[$i]['goods_code'] . " ");
                //if( $i == 0){
                if (($tong_id != $order_excel[$i]['order_id']) || ($i == 0) || ($tong_id == $order_excel[$i]['order_id'] && $i == count($order_excel) - 1)) {
                    ...
                    $this_biao->setCellValue('A' . ($i + 2), $order_excel[$i]['order_sn'] . " ")
                            ...
                            ->setCellValue('H' . ($i + 2), $order_excel[$i]['address'])
                            ->setCellValue('I' . ($i + 2), date('Y-m-d H:i:s', $order_excel[$i]['add_time']))
                            ->setCellValue('P' . ($i + 2), $order_excel[$i]['shenfenz'] . " ")
                            ->setCellValue('Q' . ($i + 2), $order_excel[$i]['postscript'] . " ")
                            ->setCellValue('O' . ($i + 2), $order_excel[$i]['invoice_no'] . " ")
                            ->setCellValue('R' . ($i + 2), $order_excel[$i]['buyer_id'] . " ");
                    ...
                    if ($i > 0 && ($tong_id != $order_excel[$i]['order_id'] || $i == count($order_excel) - 1)) {
                        ...
                        $objAlignA1 = $objStyleA1->getAlignment();
                        $excel_canshu->mergeCells("Q" . $tong_id_y . ":Q" . $i_ji); //合并
                        $objStyleA1 = $excel_canshu->getStyle("Q" . $tong_id_y);
                        $objAlignA1 = $objStyleA1->getAlignment();
                        $objAlignA1->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);  //上下居中
                        $excel_canshu->mergeCells("R" . $tong_id_y . ":R" . $i_ji); //合并
                        $objStyleA1 = $excel_canshu->getStyle("R" . $tong_id_y);
                        $objAlignA1 = $objStyleA1->getAlignment();
                        $objAlignA1->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);  //上下居中
                    }
                    $tong_id_y = $i + 2;
            ...
            $p = new Page($count, 20);
            $items_list = $items_mod->field("*,wei_nickname,(select sum(pop) from wei_pop where order_sn= sk_orders.order_sn) fp_price")->where($where)->join(" sk_dpgou on sk_dpgou.orderid = sk_orders.order_id")->join(" sk_member on sk_member.open_id=sk_orders.buyer_id ")->limit($p->firstRow . ',' . $p->listRows)->order($order_str)->select();
            foreach($items_list as &$v){
                $v['product_type']=isset($orderGood)&&($v['is_member']==1)?'会员产品':'普通产品';
            }
            $page = $p->show();
...
//         echo $logs_mod->getLastSql();die;
        ...
        $sfz_data = $info_mod->where("order_id=" . $id)->join(" sk_member on sk_member.open_id = sk_orders.buyer_id")->field(array('sk_member.sfz_zm' => 'a', 'sk_member.sfz_fm' => 'b', 'sk_orders.sfz_zm' => 'c', 'sk_orders.sfz_fm' => 'd'))->find();

        /*
         * @author csb 8/15
         */
//         dump($info);
        $openid = $info['buyer_id'];

        //通过递归调用找出顶级ID
        $where['openid'] = $this->getPartent($openid);
        $where['mac'] = $info['mac_address'];
        if (!$where['mac']) {
            unset($where['mac']);
        }
        if ($where['openid']) {

            $storeName = M('erweima', 'sk_')->where($where)->field('dianpname')->find();
            $this->assign('storeName', $storeName['dianpname']);
        } else {
            $this->assign('storeName', '没有上级!');
        }

        $order_sn = $info['order_sn'];
//         dump($order_sn);
        $income = $this->pop_test($order_sn);
//         dump($income);
        $total_pop = array();
        foreach ($income['parent'] as $v) {

            $income = explode('/', $v);
            $openid = $income[0];
            $pop = $income[1];
            $mac = $income[2];
            $data = M('member', 'sk_')->where(array('open_id' => $openid))->field('wei_nickname,wei_username,open_id')->find();
            $data['pop'] = $pop;
            $total_pop[] = $data;
        }

        $this->assign('total_pop', $total_pop);
    ...
    public function getPartent($openid, $type = 0) {


        if ($type) {
            $pid = M('member', 'sk_')->where(array('open_id' => $openid))->find();
        } else {
    ...
    public function status() {
        ...
        switch ($action) {
            ...
            case 'tradefinished':
                ...
                $sql = "select a.buyer_id,a.status,a.order_sn,a.is_member,sum(if(c.zhuangtai!=3,1,0)) kd from sk_orders a left join sk_order_goods b on a.order_id=b.order_id left join sk_kuaidi c on b.shipping_name=c.name and b.invoice_no=c.danhao where a.order_id='$id'";
                    ...
                    if ($shangopenid != "" && $arr_kdzt[0]['is_member'] == 0) {
                        ...
                        while ($i <= 3) {
                            ...
                            if (!$tru_fal && $i == 1 && ($totalPrice * 0.2) > 0 && $cunchu[0]['pid'] != $arr_kdzt[0]['buyer_id']) {//店铺商家占有利益分成
                                $sql = "insert into wei_pop (order_sn,openid,pop,poptime,duitag,shijianc,mac_address) values ('$out_trade_no','" . $cunchu[0]['pid'] . "','" . ($totalPrice * 0.2) . "' ,now(),'0'," . time() . ",'" . $mac . "')";
                                ...
                                if (!empty($agent)) {
                                    M()->query("insert into wei_pop (order_sn,openid,pop,poptime,duitag,shijianc,mac_address) values ('$out_trade_no','" . $agent['agentid'] . "',1,now(),'0'," . time() . "," . $mac . ")");
                            ...
                            if (count($havaArr) < $dp_fencheng && $fenpei > 0 && ($resultArr[0]["jointag"] == "1" || $resultArr[0]['jointag'] == 2)) {//判断上级是否有资格参与分成 且上级还未参与该订单的分成
                                $sql = "insert into wei_pop (order_sn,openid,pop,poptime,duitag,shijianc,mac_address) values ('$out_trade_no','" . $resultArr[0]["open_id"] . "','$fenpei' ,now(),'0'," . time() . "," . $mac . ")";
                                $nullmodel->execute($sql);
                                $sql = "update sk_member set shouyi = shouyi + " . $fenpei . " where open_id='" . $resultArr[0]["open_id"] . "'";
                                $nullmodel->execute($sql);

                                $weixintxt = "亲爱的 " . $resultArr[0]["wei_nickname"] . "， " . $arr_kdzt[0]['wei_nickname'] . "  购买了商城宝贝，您获取会员收益  " . $fenpei . " 元。有人问我，成为洋仆淘会员并获得收益是种怎样的感受？我只想说，you can you up ，no can 问客服~";
...
//        }
        ...
        if (!empty($_GET['daochu'])) {
            ...
            import("ORG.Util.Page");
            $count = $items_mod->table('wei_exchange exc')->where($where)->count();
    ...
    public function extstatus() {
        ...
        switch ($acton) {
            case "agree":
                ...
                $weixintxt = "亲爱的 " . $quopenid[0]['wei_nickname'] . "，您于 " . date('Y-m-d H:i:s', time()) . " 在洋仆淘商城申请提现 " . $quopenid[0]['totpop'] . " 元，已通过审核";
            ...
            case "disagree":
                ...
                $weixintxt = "亲爱的 " . $quopenid[0]['wei_nickname'] . "，很遗憾的告诉您，您 " . date('Y-m-d H:i:s', time()) . " 提现 " . $quopenid[0]['totpop'] . " 元，提现失败。";
                $this->weixintishi($quopenid[0]['openid'], $weixintxt);
                M()->query("insert into sk_message(intro,create_time,rec,type) value ('$weixintxt','" . time() . "','" . $quopenid[0]['openid'] . "',2)");
            ...
            case "money":
                ...
                $quopenid = $mod->table('wei_exchange')->join('sk_member on sk_member.open_id=wei_exchange.openid')->where(array('id' => $id))->select();
                $weixintxt = "亲爱的 " . $quopenid[0]['wei_nickname'] . "，恭喜您于 " . date('Y-m-d H:i:s', time()) . " 从洋仆淘世纪提现 " . $quopenid[0]['totpop'] . " 元，提现成功。";
                $this->weixintishi($quopenid[0]['openid'], $weixintxt);
                M()->query("insert into sk_message(intro,create_time,rec,type) value ('$weixintxt','" . time() . "','" . $quopenid[0]['openid'] . "',2)");
                break;
        }
    ...
    public function poprmb_upd() {
        $sk_pop_log = M('pop_log');
        $this->role_mod = D("role");
        //获取用户角色
        $admin_level = $this->role_mod->field('id', 'name')->where('id=' . $_SESSION['admin_info']['role_id'] . '')->find();
        $map_z['time'] = date('Y-m-d h:i:s');
        $map_z['cont'] = $_POST['order_beiz_text'];
        $map_z['user'] = $admin_level['name'];
        $map_z['pid'] = $_POST['id'];
        $sk_pop_log->add($map_z);


        $order = M();
        $map['id'] = array('eq', $_POST['id']);
        $map_x['rmb_beiz'] = $_POST['order_beiz_text'];
        $map_x['return_visit'] = $_POST['return_visit'];
        $map_x['last_visit_time'] = time();
        $data = $order->table('wei_exchange')->where($map)->save($map_x);
        echo $data;
    }

    //新添加的查看id时显示所有备注
    public function listedd() {
        $sk_pop_log = M('pop_log');
        $id = $_GET["id"];
        //找到这一条数据
        $admin_level = $sk_pop_log->where("pid=$id")->select();
        //echo $sk_pop_log->getLastSql();
        /* print_r($admin_level);
          exit; */
    ...
    function editkd() {
        ...
        if ($this->isPost()) {
            ...
            $order_openid = M()->query("select goods_name,sk_order_goods.invoice_no,buyer_id,sk_orders.shipping_name from sk_order_goods LEFT join sk_goods on sk_order_goods.goods_id=sk_goods.id left join sk_orders on sk_orders.order_id=sk_order_goods.order_id left join sk_member on sk_member.open_id=sk_orders.buyer_id
where sk_orders.order_id='$id'");

            /* dump($order_openid);exit(); */
//            //实例化表
            $orders_goods = M('order_goods');
//            /*var_dump($orders_goods);
//            die;*/
            //实例化另一个表
            $orders = M('orders');
            //快递单号
...
//            );
            ...
            /* echo 11111;exit; */
            if (isset($_POST['oid'])) {
                $id = isset($_POST['oid']) && intval($_POST['oid']) ? intval($_POST['oid']) : $this->error('请选择要编辑的链接');
            }

            /* if($shipping_name=='ems'){
              $shipping_name="EMS";
              }esleif($shipping_name=='kingfreight'){
              $shipping_name="貨運皇";
              }esleif($shipping_name=='pcaexpress'){
              $shipping_name="PCA Express";
              }esleif(){
            ...
            switch ($shipping_name) {
                ...
                case 'pcaexpress':
                    ...
                    break;
                case 'youzhengguoji':
                    $shipping_name = "邮政小包（国际";
                    break;
                case 'youzhengguonei':
                    $shipping_name = "邮政小包（国内）";
                    break;
                case 'yunda':
                    $shipping_name = "韵达快运";
                    break;
                ...
                case 'epanex':
                    ...
                    break;
                case 'lmfex':
                    $shipping_name = "良藤国际速递";
                    break;
            }
//                                            <option value="ems">EMS</option>
//                                            <option value="kingfreight">貨運皇</option>
//                                            <option value="pcaexpress">PCA Express</option>
//                                            <option value="youzhengguoji">邮政小包（国际）</option>
//                                            <option value="youzhengguonei">邮政小包（国内）</option>
...
//                                            <option value="tiantian" selected="">天天快递</option>
//                                            <option value="quanfengkuaidi">全峰快递</option>
//                                            <option value="huitongkuaidi">百世汇通</option>
//                                            <option value="disifang">递四方快递</option>
//                                            <option value="youshuwuliu">优速物流</option>
            /*  var_dump($shipping_name);exit; */
            if ($orders_goods->create()) {
                if ($orders_goods->save() !== false) {
                    $weixintxt = "您在洋仆淘购买的：" . $order_openid[0]['goods_name'] . "  快递单号： " . $invoice_no . "  快递名称：" . $shipping_name . " 快等着您的小宝贝的到来吧! 物流查询请戳：http://www.kuaidi100.com";
                    $this->weixintishi($order_openid[0]['buyer_id'], $weixintxt);
                    echo 1;
                } else
                    echo 0;
    ...
    function order_count() {
        ...
        //数量
        $where = "order_time BETWEEN '" . $start . "'and'" . $end . "'and status = 0 ";
        $count = $orders->where($where)->count();
        $amount = $orders->where($where)->sum('order_amount');
        //总额
        $where = "order_time BETWEEN '" . $start . "'and'" . $end . "'and status in (1,2,3,4) ";
        $order_count = $orders->where($where)->count();
        $order_amount = $orders->where($where)->sum('order_amount');
        //赋值
        ...
        for ($i = 0; $i <= $diffDay; $i++) {
            for ($j = 0; $j < 24; $j++) {
                $h = $j;
                if ($j < 10) {//10以下的数字1和01有区别。
                    $h = "0" . $j;
                }
                $date = date("Y-m-d $h", (strtotime($start) + $i * 24 * 60 * 60));
                $arr[$date] = array("xd" => 0, "fd" => 0, "xdmoney" => 0, "fdmoney" => 0);
            }
        }

        //匹配遍历
        $map = "order_time BETWEEN '" . $start . "'and'" . $end . "'";
        $result = $orders->field("status,order_time,order_amount")->where($map)->select();
        //echo $orders->getlastsql();exit;
        foreach ($result as $key => $v) {
            $date = date("Y-m-d H", strtotime($v['order_time']));
            if (in_array($v['status'], array(1, 2, 3, 4))) {
                $arr[$date]['fd'] ++;
                $arr[$date]['fdmoney']+=$v['order_amount'];
            } else {
                $arr[$date]['xd'] ++;
                $arr[$date]['xdmoney']+=$v['order_amount'];
            }
        }
        //转换json，图表使用
        $chart = array();
    ...
    public function pop_test($out_trade_no) {

        //收益分配-----------------------------------------------------------------------
        $do = M();
        $totalPrice = 0; //商品分成的价格
        //查询mac地址
        $mac_sql = "select mac_address from sk_orders WHERE order_sn='$out_trade_no'";
        $mac_data = $do->query($mac_sql);
        $mac = $mac_data[0]['mac_address'];
        ...
        $sql = "select fp_price from sk_goods where id = '$goods_id'";
        $ug_price = $do->query($sql);
//             dump($ug_price);
        //判断商品是否有利润可分
        if ($ug_price[0]['fp_price'] > 0) {
            $totalPrice = floatval($ug_price[0]['fp_price']);
            $havaDaili = true; //只要代理产品了，即修改客户身份
        }
        ...
        //获取当时利润分配
        $i = 1;
        $sql = "select m.pid,m.open_id,m.wei_nickname,o.order_id,o.is_shouyi,o.syq_price,o.goods_amount,o.order_amount,o.couponid,o.mcouponid,o.discount,o.is_member from sk_orders o left join sk_member m on o.buyer_id=m.open_id where o.order_sn='$out_trade_no'";
        $resuArr = $do->query($sql);
        ...
        if (!$havaDaili) {
            if ($resuArr[0]['is_member'] == 1) {
                //购买商品可以获得会员
                $arr_count['join_member'] = 1;
            }
        }
        //$havaDaili为true代表有收益可分
        if ($havaDaili == true) {//用户购买了代理商品，修改用户信息
            ...
            if ($shangopenid != "") {
                $flag = true;
                $cunchu = $this->getPartent($resuArr[0]['open_id'], 1); //获取顶端店家
                $cunchu = M('member', 'sk_')->field('jointag,pid')->where(array('open_id' => $cunchu))->select();
                if ($cunchu[0]['pid'] == "" && $cunchu[0]['jointag'] == 2) {//最顶端不是店家则执行
                    $flag = false;
                }

                while ($i <= 3) {

                    $sql = "select * from sk_member where open_id='$shangopenid'";
                    $resultArr = $do->query($sql);
                    if (count($resultArr) == 0)
                        break;

                    //更换openid为pid下次循环的时候继续查上级
                    $shangopenid = $resultArr[0]["pid"];
                    ...
                    if (count($havaArr) < $dp_fencheng && $fenpei > 0 && ($resultArr[0]["jointag"] == "1" || $resultArr[0]['jointag'] == 2)) {//判断上级是否有资格参与分成 且上级还未参与该订单的分成
                        //$tru_fal为true代表他的最顶级有店铺商家,$i == 1 && !flag 代表为商家
                        if (!$flag && $i == 1 && ($totalPrice * 0.2) > 0) {//店铺商家占有利益分成 2成
                            //商家获取2成收益 3级盟友 0.5 0.2 0.1
                            $arr_count['parent'][$i] = $resultArr[0]["open_id"] . '/' . ($totalPrice * 0.2 + $fenpei) . '/' . $mac;
                        } else {
                            $arr_count['parent'][$i] = $resultArr[0]["open_id"] . '/' . $fenpei . '/' . $mac;
                        }
                    } else {
                        $i = 100; //跳出循环
                    }

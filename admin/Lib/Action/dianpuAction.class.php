...
class dianpuAction extends baseAction {
    ...
    function geren_dianpu() {
        ...
        //$tj['sk_orders.status'] = 1;
        $tj['_string']="  sk_orders.status =1 or  sk_orders.status =4";
        //$tj['sk_orders.order_time'] = array('lt', "2016-08-26 14:00:21");

        //店铺导出Excel
        if (!empty($_GET['daochu'])) {

            $field = 'wei_nickname,openid,dianpname,attribute,dianp_num,xxdizhi,mac,regional,proxy,storeboss,macpayer,peixun,docking,starttime,budian';
            $excel = M('erweima')->join('LEFT JOIN sk_member ON sk_erweima.openid=sk_member.open_id')->join(' LEFT JOIN sk_orders on sk_erweima.dingdan=sk_orders.order_id')->order(array("sk_erweima.id" => 'desc'))->where($tj)->field($field)->select();
            ...
            $excel_canshu->getColumnDimension('E')->setWidth(15);
            $excel_canshu->getColumnDimension('F')->setWidth(25);
            ...
            $excel_canshu->getColumnDimension('K')->setWidth(25);
            ...
            $this_biao->setCellValue('A1', '微信昵称')
                    ...
                    ->setCellValue('D1', '店铺性质')
                    ->setCellValue('E1', '店铺联系方式')
                    ->setCellValue('F1', '详细地址')
                    ->setCellValue('G1', '终端透明柜MAC')
                    ->setCellValue('H1', '区域负责人')
                    ->setCellValue('I1', '代理人信息')
                    ->setCellValue('J1', '实体店老板')
                    ->setCellValue('K1', '终端付款人')
                    ->setCellValue('L1', '是否培训')
                    ->setCellValue('M1', '店铺实际对接人')
                    ->setCellValue('N1', '店铺启动时间')
                    ->setCellValue('O1', '布点人');                    
            $i = 2;
            foreach ($excel as $k => $v) {
                ...
                $objPHPExcel->setActiveSheetIndex(0)
                        ...
                        ->setCellValue('D' . $i, $v['attribute'])
                        ->setCellValue('E' . $i, $v['dianp_num'])
                        ->setCellValue('F' . $i, $v['xxdizhi'])
                        ->setCellValue('G' . $i, $v['mac'])
                        ->setCellValue('H' . $i, $v['regional'])
                        ->setCellValue('I' . $i, $v['proxy'])
                        ->setCellValue('J' . $i, $v['storeboss'])
                        ->setCellValue('K' . $i, $v['macpayer'])
                        ->setCellValue('L' . $i, $peixun)
                        ->setCellValue('M' . $i, $v['docking'])
                        ->setCellValue('N' . $i, $v['starttime'])
                        ->setCellValue('O' . $i, $v['budian']);                        
                $i++;
            }
            ...
            import("ORG.Util.Page");
            $Page = new Page($count, 20);
            $dianps = $dianp->field("sk_erweima.*,sk_orders.order_id,(select count(*) from sk_role_store b where sid=id and b.agentid='$agentid' and b.pid=openid) sel")->join('sk_orders on sk_erweima.dingdan=sk_orders.order_id')->join("sk_role_store c on pid=openid and sid=id")->order(array("sel" => 'desc', "id" => 'desc'))->where($tj)->limit($Page->firstRow . ',' . $Page->listRows)->select();
//            echo  $dianp->getLastSql();
            $show = $Page->show();
            $this->assign('page', $show);
            $this->assign('get', $_GET);
            $this->assign('items_cate_list', $dianps);
            $this->display();
        }
    }
    ...
    function dianpuchakan() {
        ...
        $dianpu = $dianpu->where($whe)->join('sk_member on sk_member.open_id = sk_erweima.openid')->select();
        $this->assign('items_cate_list', $dianpu);
        $openid = $dianpu[0]['openid'];
        $mac = $dianpu[0]['mac'];
        if (!empty($mac)) {
            //$link = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx8b17740e4ea78bf5&redirect_uri=http%3A%2F%2Fm.hz41319.com%2Fwei%2Flogin.php&response_type=code&scope=snsapi_userinfo&state=" . $info['openid'] . "|shangp|" . $this->sid . "|mac_" . urlencode($info["mac"]) . "&connect_redirect=1#wechat_redirect";
            $link = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx8b17740e4ea78bf5&redirect_uri=http%3A%2F%2Fm.hz41319.com%2Fwei%2Flogin.php&response_type=code&scope=snsapi_userinfo&state=" . $openid . "|mingpian|mac_" . urlencode($mac) . "&connect_redirect=1#wechat_redirect";
        } else {
...
//                                        echo $result."---".$count; die;
                    ...
                    //修改用户为店铺会员
                    $member = D("member");
                    $member_data = $member->field("e.id,open_id,mac_address,jointag,e.mac")->join(" sk_erweima e on  open_id=openid")->where(" e.id='" . $_POST['id'] . "'")->find();

                    if (empty($member_data['mac_address'])) {
                        $member->where("open_id='" . $member_data['open_id'] . "'")->setField('mac_address', $member_data['mac']);
                    }
                    if ($member_data['jointag'] != '2') {
                        $member->where("open_id='" . $member_data['open_id'] . "'")->setField('jointag', '2');
                    }
                }
                $this->success('修改成功');
            } else
    ...
    function checkmac() {
        ...
        $dianpu = D("erweima");
        $count = $dianpu->where("mac='" . $mac . "'")->count();
        if ($count > 0) {
            echo 1;
        } else {
            echo 0;

...
class goodsAction extends baseAction {
	public function index() {
		...
		$gys_bianma = isset($_GET['gys_bianma']) && trim($_GET['gys_bianma']) ? trim($_GET['gys_bianma']) : '';
		...
		if ($gys_bianma) {
			$where .= " AND sk_goods_gongying_cangku.cangku_bianhao in(select cangku_bianhao from sk_goods_gongying_cangku where gongying_bianhao  LIKE '%" . $cangku_bianhao . "%') ";
			$this->assign('gys_bianma', $gys_bianma);
		}    
		if ($cangku_bianhao) {
			$where .= " AND sk_goods_gongying_cangku.cangku_bianhao LIKE '%" . $cangku_bianhao . "%'";
			$this->assign('cangku_bianhao', $cangku_bianhao);
		} 
		if ($gys_name) {
			$where .= " AND sk_goods_gongying_cangku.cangku_bianhao in(select cangku_bianhao from sk_goods_gongying_cangku where gongying_name  LIKE '%" . $gys_name . "%') ";
			$this->assign('gys_name', $gys_name);
		}    
		if ($cangku_name) {
			$where .= " AND sk_goods_gongying_cangku.cangku_name LIKE '%" . $cangku_name . "%'";
		...
		if($goodtype==99)
		  ...
		  $goodtype >= 0 && $where .= " AND sort_order not in(1,2,3,4,5) AND goodtype=" . $goodtype;
		...
		if(!empty($_GET['daochu'])){//-> join("sk_items_cate on sk_items_cate.id = sk_goods.cate_id")
    		...
    		$count = $items_mod -> join(' sk_goods_gongying_cangku ON sk_goods.cangku_bianhao = sk_goods_gongying_cangku.cangku_bianhao') -> join(' sk_goods_gongying ON sk_goods_gongying_cangku.gongying_bianhao = sk_goods_gongying.gys_bianma') ->where($where)->count();
    		$p = new Page($count, 20);
    		$items_list = $items_mod -> join(' sk_goods_gongying_cangku ON sk_goods.cangku_bianhao = sk_goods_gongying_cangku.cangku_bianhao') -> join(' sk_goods_gongying ON sk_goods_gongying_cangku.gongying_bianhao = sk_goods_gongying.gys_bianma') ->where($where)->limit($p->firstRow . ',' . $p->listRows)->order($order_str)->select();
    ...
    public function edit()
    {
        ...
        if (isset($_POST['dosubmit'])) {
            ...
            $cangku_bianhao = isset($_POST['cangku_bianhao']) && trim($_POST['cangku_bianhao']) ? trim($_POST['cangku_bianhao']) : '';
            if($cangku_bianhao){
                $gys_data['cangku_bianhao'] = $cangku_bianhao;
    ...
    //                }
         ...
         /*            $content = $_REQUEST['good_name'];
                    ...
                    if ($_FILES['tuan_img']['name'] != '') {
                        $upload_list = $this->_upload($_FILES['tuan_img']);
                        $items_mod->tuan_img = $items_mod->simg= $items_mod->bimg = 'data/goods/m_' . $upload_list['0']['savename'];
                    }
                    if ($_FILES['tuan_img_jiage']['name'] != '') {
                        $upload_list = $this->_upload($_FILES['tuan_img_jiage']);
                        $items_mod->tuan_img_jiage = $items_mod->simg= $items_mod->bimg = 'data/goods/m_' . $upload_list['0']['savename'];
                    }
                    $gos_m = M('goods');
  ...
  /*
    ...
    */
    ...
    public function sg(){
        ...
        if($this->ispost()){
            if($sg->create()){
                $sg->xs_time=strtotime($sg->xs_time);
                if($sg->add()){
                    $this->success('添加成功');
                }else {
                    $this->error('添加失败');
        ...
        $sglist=$sg->order('xs_time desc')->select();
    ...
    public function sg_edit(){
        ...
        if($data=$sg->find(intval($_GET['id']))){
            $data['xs_time']=date('Y-m-d H:i:s',$data['xs_time']);
            $this->assign('sg',$data);
        }else {
            $this->error('没有此闪购安排');
        }

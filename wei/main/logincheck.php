...
file_put_contents('logfile.log', date("Y-m-d H:i:s"). " " . "用户的:".$openid.PHP_EOL, FILE_APPEND | LOCK_EX); //记录日志
if($openid==""){
	echo "{\"success\":\"0\"}";
}else{
	...
	$sql = "select jf from sk_member where open_id='$openid'";
        file_put_contents('logfile.log', date("Y-m-d H:i:s"). " " . "用户的:".$openid."  --sql:".$sql.PHP_EOL, FILE_APPEND | LOCK_EX); //日志
...
//			}
		...
		if(date("Y-m-d",$info['last_login'])!=date("Y-m-d"))
			$do->dealsql("update sk_member_count set count=count+1 where time='".date("Y-m-d")."'");//每日统计访问量
                ...
                file_put_contents('logfile.log', date("Y-m-d H:i:s"). " " . "用户的:".$openid."--sql:"." update sk_member_count set count=count+1 where time='".date("Y-m-d")."'".PHP_EOL, FILE_APPEND | LOCK_EX); //日志

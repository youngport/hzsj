...
$sql = "select * from sk_jrhdlfx where hd_youxiao = 1 limit 0,1";
$jrhd_fenxiang = $do -> selectsql($sql);
?>
<!DOCTYPE html>
...
wx.ready(function(){
	var shareData = {
		...
		imgUrl: 'http://<?php echo $_SERVER['HTTP_HOST'].$jrhd_fenxiang[0]['img'] ?>',
...
function huodongye(d,l){
	window.open("huodong.php?d="+d+"&l="+l);
}
function huod(){
	$.ajax({
		type: "POST",
		...
		success: function(data){
			...
			if(data.length>0){
				...
				for(var i=0;i<data.length;i++){
					appStr += "<li onclick=\"huodongye("+data[i].id+","+data[i].lei+");\"><img src=\"<?php echo $cxtPath ?>/wei/css/img/shangou_03.jpg\" class=\"imgtt\"/>";
					appStr += "<img src=\"<?php echo $cxtPath ?>"+data[i].img+"\" class=\"imgtt1\"/>";
					appStr += "<a></a></li>";
	...
	$.ajax({
		...
		success: function(data){
			...
			if(data.length>0){
				appStr = "";
				for(var i=0;i<data.length;i++){
					appStr += "<div class=\"tuandange\"><a href=\"tuangou_xq.php?kd="+data[i].id+"\"><img src=\"<?php echo $cxtPath ?>/"+data[i].tuan_img+"\" class=\"imgtt\"/></a>";
					...
					appStr += "<p style=\"font-size:22px;\">￥"+Number(data[i].tuangoujia)+"</p>";
					appStr += "<span>再省"+(data[i].orgprice - data[i].tuangoujia)+"元</span>";
					appStr += "<span>包邮</span>";
					appStr += "</div>";
					appStr += "</div><div style=\"clear:both;\"></div></div>";
				}

...
function buy(){
	...
	$.ajax({
		type: "POST",
		url: "goodsAddOrder123.php",
		data: "goodsid=<?php echo $id ?>&guige="+guige+"&ms=<?php if($huodong){echo 1;}else{echo 2;}?>&color="+color+"&tax_rate="+tax_rate+"&sul="+$('#number').val(),
		success: function(data){
			data = eval("("+data+")");
			if(data.success == "1"){
				//window.open('../order/order.php?orderid='+data.orderid);
				location.href='../order/order123.php?orderid='+data.orderid;
			}else if(data.success == "2"){
				msgError("该商品库存不足");
			}else if(data.success == "3") {
				msgError("该商品不在活动时间段，操作失败");
...
<?php }else if($jointag =="2"){?>
	 <div class="spxq_jiage"><span class="danjia">￥<?php echo $goodArr[0]["price"]?></span><span style="color:#fff;font-size: 1.3em;background: url(../../img/hd_bq.png);padding: 1px 10px;"><?php if($_GET['cate']=='88'){ echo '批发价'; }else{ echo '批发价';} ?></span><span class="yuanjia">市场价：￥<?php echo $goodArr[0]["orgprice"] ?></span></div>
<?php }else{?>
...
<?php if($huodong){?>
	<div class="spxq_top"><span>进口税：<font style="color:#ce8d1b" id="tax_rate"><?php echo round($tax_rate* $goodArr[0]["huodongjia"],2)?>元</font><img src="../../img/jks.png" style="    width: 15px;height: 15px;
margin-left: 5px;" onclick="Layer()"></span>
<?php }else if($goodArr[0]["price"]*$tax_rate<50  && $goodArr[0]["fh_type"] == "直邮"){?>
<div class="spxq_top"><span>进口税：<font style="color:#ce8d1b" id="tax_rate">0.00元</font><img src="../../img/jks.png" style="    width: 15px;height: 15px;
margin-left: 5px;" onclick="Layer()"></span>
<?php }else if($goodArr[0]["huodongjia"]*$tax_rate<50  && $goodArr[0]["fh_type"] == "直邮" && $huodong){?>
<div class="spxq_top"><span>进口税：<font style="color:#ce8d1b" id="tax_rate">0.00元</font><img src="../../img/jks.png" style="    width: 15px;height: 15px;
margin-left: 5px;" onclick="Layer()"></span>
<?php }else{?>
	<div class="spxq_top"><span>进口税：<font style="color:#ce8d1b" id="tax_rate"><?php echo round($tax_rate*$goodArr[0]["price"],2)?>元</font><img src="../../img/jks.png" style="    width: 15px;height: 15px;
margin-left: 5px;" onclick="Layer()"></span>
<?php }?>
...
<li>退换货：不支持无理由退换货</li>
<li>进口税：按照最新税改征收<?php echo $tax_rate*100 ?>%税率(直邮税收不足50元免税)</li>
...
<table class="rigou" style="position: fixed;bottom: 0px;">
	<tr align="center">
	<?php if($id_goodstype==3){?>

...
include '../../../base/commonFun.php';
	...
	if($notify->checkSign() == TRUE)
	{
		...
		elseif($notify->data["result_code"] == "FAIL"){
			...
			$i = 1;
			$sql = "select m.pid,m.open_id,m.wei_nickname,o.order_id,o.goods_amount,o.couponid,o.mcouponid,o.discount from sk_orders o left join sk_member m on o.buyer_id=m.open_id where o.order_sn='$out_trade_no'";
			$resuArr = $do->selectsql($sql);
						...
						if($shangopenid != ""){
						    ...
						    while($i<=3){
						        ...
						        if(count($havaArr) < $dp_fencheng && $fenpei > 0 && ($resultArr[0]["jointag"]=="1"||$resultArr[0]['jointag']==2)){//判断上级是否有资格参与分成 且上级还未参与该订单的分成
						            ...
						            $do->dealsql($sql);
									$weixintxt="亲爱的 ".$resultArr[0]["wei_nickname"]."， ".$resuArr[0]['wei_nickname']."  加入会员，您获取会员收益  ".$fenpei." 元。有人问我，成为和众会员并获得收益是种怎样的感受？我只想说，you can you up ，no can 问客服~";

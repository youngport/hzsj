...
<div>
	<div class="container-fluid" id="list" style="overflow-y:scroll;">
		<table class="list">
			...
			$time=time();
			$list='';
			echo "select * from sk_coupon where rec='$openid' and status in(1,3) and (type=0 or (type=1 and tid in ($cates)) or (type=2 and tid in ($goods))) and start_time<=$time and end_time>=$time and xz<=".$order['goods_amount']." order by end_time desc";exit;

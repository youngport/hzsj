...
/////////$orderid
$jointag = $do->getone("select jointag,shouyi,post_code,wei_shousfz from sk_member where open_id='$openid'");
//print_r($jointag);exit;

$post_code = $jointag['post_code'];
$wei_shousfz = $jointag['wei_shousfz'];
$sql = "SELECT sk_order_goods.rec_id,sk_goods.fh_type FROM sk_order_goods join sk_goods on sk_order_goods.goods_id=sk_goods.id WHERE sk_order_goods.order_id = '$orderid' and sk_goods.cate_id = 88";
$coulist = $do->getone("select mcouponid,couponid from sk_orders where order_id=$orderid");
$naifen_youfei = $do->selectsql($sql);
$ty = $do->getone("select fh_type from sk_goods g inner join sk_order_goods o on g.id=o.goods_id where order_id = '$orderid'");
$arr_rate  = $do->selectsql("select tax_rate tax_rate,quantity from sk_order_goods WHERE order_id = '$orderid'");
//print_r($arr_rate );exit;
                    ...
                    success: function (data) {
                        ...
                        for (var i = 0; i < data.length; i++) {
                            ...
                            var price = data[i].price;
                             var tax_rate = data[i].tax_rate;
                            jifen = data[i].jifendh;

                            var appStr = "<li>";
                            appStr += "<img src=\"<?php echo $imgurl ?>" + data[i].img + "\">";
                            if (jifen == 1){
                                appStr = "<div class=\"hg\"><p class='my-p'>" + data[i].good_name + "</p><div class=\"jia clearfix\"><font>积分</font><b>" + price + "</b><span>" + quantity + "</span></div>";
                            }
                            else{
                                <?php if($ty['fh_type']=='直邮') { ?>
                                appStr += "<div class=\"hg\"><p class='my-p'>" + data[i].good_name + "</p><div class=\"jia clearfix\" style=\"color:#DAA958\">行邮税&nbsp&nbsp" + tax_rate + "元<div class=\"jia clearfix\"><font>¥&nbsp;</font><b>" + price + "</b><span>" + quantity + "</span></div>";
                                <?php }else{?>
                                appStr += "<div class=\"hg\"><p class='my-p'>" + data[i].good_name + "</p><div class=\"jia clearfix\" style=\"color:#DAA958\">跨境综合税&nbsp&nbsp" + tax_rate + "元<div class=\"jia clearfix\"><font>¥&nbsp;</font><b>" + price + "</b><span>" + quantity + "</span></div>";
                                <?php }?>
                            }
                        ...
                        if (jifen == 1) {
                            ...
                            if (huiyuan == 1 || huiyuan == 2 ||hy==1) {
                                $('.coupon').hide();
                            }
                            var naifen_youfei =<?php echo count($naifen_youfei); ?>;
                            if (zquantity >= 2 || total >= 299 || hy==1 || huiyuan == 1 || huiyuan == 2 || naifen_youfei > 0 ||<?php echo empty($mcoupon) ? 0 : 1; ?> > 0) {
                                $('#hanyunfei').html('合计:');
                            } else {
                                total += 10;
                            }
                            ...
                            if ("<?php echo empty($mcoupon) ? 0 : 1; ?>" > 0) {
                                ...
                                total +=  <?php echo $tax_rate ?>;
                            $("#totalPrice").text("￥" + toDecimal2(total));
                            $("#by").text("￥" + toDecimal2(total));
...
/*                if (shenfenz == "" ) {
                ...
                if (jifen == 1) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "orderPay123.php",
                        data: "orderid=<?php echo $orderid ?>&shouname=" + shouname + "&phone=" + phone + "&address=" + address + "&shenfenz=" + shenfenz + "&name=" + name + "&post_code=" + post_code + "&shouyi=" + shouyi + "&postscript=" + postscript,
                        success: function (data) {
                            data = eval("(" + data + ")");               
                            if (data.success == "1") {
                                location.replace("../weipay/demo/js_api_call123.php?order=" + data.order_id + "&is_sy=" + data.is_shouyi);
                            } else {
                                msgError("购买失败，请稍后重试");
                            }
                        }
...
<!--                <li><img src="img/luhui.jpg">
            ...
            <ul class="u2 clearfix" >
                            <li class="jinkou_count" style="background:none">
                    <a id="jinkou_count" >
                      <?php if($ty['fh_type']=='直邮') { ?>
                        行邮税总计<span style='float: right;margin-right: 3%;font-size: 16px;color: #DAA958;'>¥<?php echo $tax_rate ?>元</span>
                       <?php }else{?>
                       跨境综合税总计<span style='float: right;margin-right: 3%;font-size: 16px;color: #DAA958;'>¥<?php echo $tax_rate ?>元</span>                      
                       <?php } ?>
                    </a>
                </li>
                <li class="li-first">
                ...
                <li>
                    <a id="addDiv1" style="display:none;" >                      
                        <p style="line-height: 2em;">收货人：<font id="shouname"><?php echo $wei_shouname ?></font>&nbsp;&nbsp;&nbsp;<span id="phone" style="line-height:0px;display:inline-block;font-size:12px;"><?php echo $phone ?></span></p>
                        <p style="line-height: 2em;">收货地址：<font id="address"><?php echo $address ?></font></p>
                        <p style="line-height: 2em;">邮编：<font id="post_code"><?php echo $post_code ?></font></p>
                    </a>
                </li>
                <li><span style="font-size:15px;padding: 0 0;">姓名：<input type="text" id="sf_name"class="text-input" value="<?php echo $wei_shouname ?>"/></span></li>
                <li><span style="font-size:15px;">身份证：<input type="text" id="sf_code" name="shenfenzhneg" class="text-input" placeholder="境外商品清关需购买人的身份证号码" value="<?php echo $wei_shousfz ?>" /></span></li>
                <li><span>如有疑问，请联系客服：<font style="      white-space: nowrap;display: inline-block;">400-888-3658</font></span></li>
            <li class="mj"><span style="font-size:15px;">备注：<input type="text" name="postscript" id="postscript" class="text-input" placeholder="留言"/></span></li>
             </ul>
            <div class="dk" style="height:220px;">
        ...
        <div class="shouyi_a" id="shouyi" status="0">收益抵扣(您可使用的收益余额:<font id="s_price"><? echo $jointag['shouyi'] ?>）</font></div>
        <div style="    margin-left: 3%;color:#898989">温馨提示：点击勾选框可使用收益抵扣</div> 
        <div style="    margin-left: 3%;color:#898989">受新税改的影响商城部分商品价格近期会有调整</div> 
    </div>
            <div class="zhifu" style="background: #fff;">
            <span style="width: 45%;margin-bottom: -43px;"><b id="hanyunfei">邮费10元&nbsp;合计</b> 
            <font id="totalPrice" style="color: #eb6877;    font-size: 16px;">00.00</font></span>
            <font id="by" style="color: #eb6877;font-size: 16px;display:none">00.00</font></span>
            <span onclick="pay()" style="width: 46%;margin-left: 54%;background: #18A328;">微信支付</span>
        </div>
</div>
        <script type="text/javascript">       
$(function(){ 
    $('#shouyi').click(function(){
            ...
            if($(this).attr('class') =='shouyi_a'){
                $(this).attr('class','shouyi_b');
                $(this).attr('status','1');
                //点击使用收益抵扣 ，价格重新赋值              
                    var totalPrice =  $('#totalPrice').text();
                    totalPrice = parseFloat(totalPrice.substr(1,totalPrice.length));
                    var s_price = parseFloat($('#s_price').text());
                    totalPrice -= s_price; 
                    //alert(totalPrice);
                    if (totalPrice <= 0 ) {
                        totalPrice = 0.1;
                ...
                $(this).attr('class','shouyi_a');
                $(this).attr('status','0');
                 //点击不使用收益抵扣 ，价格重新赋值 
                    var totalPrice =  $('#by').text();
                    totalPrice = parseFloat(totalPrice.substr(1,totalPrice.length));                              
                    $("#totalPrice").text("￥" + toDecimal2(totalPrice));

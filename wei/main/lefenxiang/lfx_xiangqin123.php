...
$id = intval($_GET['spid']);
$sql = "select lfx.biaoti,lfx.neirong,lfx.show_share,lfx.shijian,lfx.openid,lfx.dianji,me.wei_nickname,me.headimgurl,(select count(*) from sk_lfx_dz where lfxid=lfx.id) dianzan,(select count(*) from sk_lfx_pinglun where lfx_id=lfx.id ) pinlun from sk_lfx lfx left join sk_member me on me.open_id=lfx.openid where lfx.id='$id'";
$data = $do->getone($sql);
/*$neirong = substr($data['neirong'],23,39);*/
...
unset($sql);
$sql = "select count(*) as dz_count from sk_lfx_dz where openid='" . $openid . "' and lfxid='" . $id . "'";
$data_dz = $do->selectsql($sql);
?>
<!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        <meta name="viewport" content="width=320, initial-scale=1.0, maximum-scale=1.0, user-scalable=no">
            ...
            <script type="text/javascript">
                ...
                wx.ready(function(){
                    var shareData = {
                        title: "<?php if($data['openid'] == '0') echo $data['biaoti']; else echo '洋仆淘O2O跨境商城－乐分享' ?>",
                        desc: "<?php if($data['openid'] == '0') echo $data['show_share']; else echo '...'; ?>", // 分享描述
                        link: 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx8b17740e4ea78bf5&redirect_uri=http%3A%2F%2Fm.hz41319.com%2Fwei%2Flogin.php&response_type=code&scope=snsapi_userinfo&state=<?php echo $openid; ?>|lfx|<?php echo $id;?>&connect_redirect=1#wechat_redirect',
                        imgUrl: '<?php echo $cxtPath."/".$imglist[0]["imgurl"] ?>',
    ...
    <body>
        <div class="dazonghe" id="dazonghe">
            <!--            <div id="top_p"></div>-->
            <div class="biaoti"><img src="<?php if ($data['openid'] == '0') {
    echo '../../img/admintouxiang.png';
} else {
    echo $data['headimgurl'];
} ?>" /><span ><?php if ($data['openid'] == '0') {
                echo '管理员';
            } elseif ($data['openid'] == '') {
                echo '匿名用户';
            } else {
                echo $data['wei_nickname'];
            } ?></span></div>
            <?php
            //var_dump($cookieArr);
...
?>
            <div class="shuxing">
                ...
                <div style="width:18%;" class="dianzan" data-dzcs="<?php echo $data['dianzan']; ?>" data-dzid="<?php echo $id; ?>"> <?php  if ($data_dz[0]['dz_count'] == 0) { ?><img data-dzid='<?php echo $id ?>' src="../../css/img/good.png"/> <?php } elseif ($data_dz[0]['dz_count'] == 1) { ?><img src="../../css/img/gooded.png"/><?php } ?>&nbsp;<span><?php echo $data['dianzan']; ?></span></div>
        ...
        $('#pinglun').click(function () {
            if ($('#contant').val() === "") {
                ...
                $.ajax({
                    ...
                    data: {"id":<?php echo $id; ?>, "neirong": $('#contant').val().replace(/\r/gi, "<br\/>").replace(/\n/gi, "<br\/>")},
                    success: function (data) {
                        if (data == 1) {
                            alert('评论成功');
                            location.reload();

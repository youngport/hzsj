...
$sql_in = $_GET['id'];
if($sql_in != ""){
    $sql = "select id,img,price,good_name from sk_goods where id in(".$sql_in.")";
...
wx.ready(function(){
	...
	<style type="text/css">
		.info_c{position: absolute;left: 105px; top:90px;font-size:14px;line-height:1em; }
		p{margin-top: 10px;}
		body{font-family: '黑体'; }
		...
		-o-border-radius:4px;
		-ms-border-radius:4px;}
		.img{margin-top: 15px;width: 80px;height: 80px;border: 1px solid #ccc;}
	</style>
</head>
<body style="background:#f4f4f4;">
<div class="top">分享</div>
<div class="nav">分享商品</div>
<div class="share">
	<div style="margin-left: 1em;height: 110px;" class="cookie_god" id="cookie_god">
	<?php 
    for($i=0;$i<count($cookieArr);$i++){
        echo "<img class= img data-id = '".$cookieArr[$i]['id']."' src='".$cxtPath."/".$cookieArr[$i]['img']."'>";
    }
?>
</div>
	<div class="info_c">
	<?php 
    for($i=0;$i<count($cookieArr);$i++){
        echo "<p>".$cookieArr[$i]['good_name']."</p>";
        echo "<p>￥".$cookieArr[$i]['price']."</p>";
    }
?>
	</div>

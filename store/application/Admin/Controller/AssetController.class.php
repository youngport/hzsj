...
class AssetController extends AdminbaseController {
	...
	function add_post(){
		if(IS_POST){
			if($data=$this->asset_model->create()){
				...
				$data['time']=time();
				if($this->asset_model->add($data))
					$this->success("添加成功");
				else
					$this->error("添加失败");
			}else
				$this->error($this->asset_model->getError());
		}
	}
	//分割上传
	function cutfile(){
		...
		$path="./data/upload/asset/".date("Y-m-d")."/";
	...
	function delete(){
		$id=I("id");
		$result=$this->asset_model->delete($id);
		if($result!==false) {
			$this->success("删除成功");
		}else
			$this->error("删除失败");
